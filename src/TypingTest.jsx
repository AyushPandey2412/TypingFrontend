...
const TypingTest = () => {
  ...
  // Check authentication on mount
  useEffect(() => {
    const userData = localStorage.getItem("typingTestUser");
    if (userData) {
      setIsAuthenticated(true);
      setUser(JSON.parse(userData));
    }
  ...
  const handleLogin = () => {
    ...
    const existingUsers = JSON.parse(localStorage.getItem("typingTestUsers") || "[]");
    const foundUser = existingUsers.find(u => u.email === loginData.email && u.password === loginData.password);

    if (foundUser) {
      const userData = { name: foundUser.name, email: foundUser.email };
      localStorage.setItem("typingTestUser", JSON.stringify(userData));
      setIsAuthenticated(true);
      setUser(userData);
      setShowLogin(false);
      setMobileMenuOpen(false);
  ...
  const handleSignup = () => {
    ...
    localStorage.setItem("typingTestUsers", JSON.stringify(existingUsers));
